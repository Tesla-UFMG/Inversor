    ...
    for (;;) {
        ...
        switch (g_control_type) {
            case LATERAL: // TODO(giovanni): do the integration of the two controllers
                ...
                log_data(ID_TORQUE_DECREASE_R, result_lateral.torque_decrease[R_MOTOR]);
                log_data(ID_TORQUE_DECREASE_L, result_lateral.torque_decrease[L_MOTOR]);
                osDelayUntil(tick);
            ...
            case LONGITUDINAL:
                ...
                send_ref_torque_message(ref_torque);
                log_data(ID_TORQUE_DECREASE_R, result_longitudinal.torque_decrease[R_MOTOR]);
                log_data(ID_TORQUE_DECREASE_L, result_longitudinal.torque_decrease[L_MOTOR]);
                osDelayUntil(tick);
